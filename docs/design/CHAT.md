...
- **compact 済みやりとり**: `subtype === 'compact_boundary'` の system レコードより前に開始した
  やりとり。棒グラフでグレー表示して区別するが、クリックによる絞り込みは通常のやりとりと
  同じにできる（Issue #55 で改定）。コスト合計にも含める（整合を崩さない）
...
- [x] `SPEC-CHAT-045` compact_boundary より前に開始したやりとりは compacted としてグレー表示で区別しつつ、クリックで絞り込みでき、コスト合計にも含める（Issue #55 改定）
...
- [x] `SPEC-CHAT-076` compact 済み（グレー）の帯もクリックで会話が絞り込まれ、解除できる（Issue #55）
