...
- [x] `SPEC-LIVE-024` 総件数が増えたとき取得済みの本文は破棄せず、末尾の部分ページは不足分だけ取得して既存ページへ合流する（Issue #33 で改定。旧文言「末尾の部分ページだけ再取得する」は表示中本文の破棄→ちらつきの原因だったため置き換え）
...
基本仕様書は [docs/spec/LIVE.md](../spec/LIVE.md) の同名セクション。サーバ側（LiveHub・SSE）は変更しない。**クライアントのみの変更。**

## 現状の問題（#33 着手時のコード確認）

1. **行の全再構築**: `applyAppend` が `detail` を差し替えるたびに `buildRows()` が全レコードを先頭から平坦化し直す。全 Row のオブジェクト参照が変わるため、仮想スクロールの可視行すべてが再レンダリングされる
2. **本文キャッシュの破棄**: `bodystore.grow()` が末尾の部分ページを丸ごと破棄する（旧 SPEC-LIVE-024 の仕様どおり）。表示中だった直近メッセージの `peek()` が `undefined` に戻り、再取得が完了するまでプレースホルダ表示になる（ちらつきの正体）

## 実装方針

### bodystore の不足分取得（SPEC-LIVE-024 改定）

ページごとに取得済み件数を管理する。`grow()` は取得済み本文を破棄しない。部分ページに属する未取得 index の `ensure()` は、**そのページの取得済み末尾から不足分だけ** `fetchPage(既取得末尾, 不足数)` で取得し、既存配列へ連結する。取得済み範囲への再リクエストは発生しない。

### 行の増分構築（thread.ts）

`createRowBuilder()` を追加し、`buildRows()` の内部状態（`branchByUuid` / `ownerByToolUseId`）を append をまたいで保持する。

- `append(records)`: 新規行は末尾へ push。既存行への取り付け（tool_result の owner 行・sidechain の分岐行への連結）は、**該当行だけを copy-on-write で新しい参照に置き換える**。変化のない行の参照は維持する
- `rows()`: 呼び出しごとに新しい配列を返す（React の変更検知用）。要素の参照は上記の規律に従う
- `buildRows(records)` は builder を 1 回使う純関数として残す（既存利用・テスト互換）

### SessionView の差分適用

- builder を ref に保持し、append イベントが `payload.start === 既知件数` のときだけ `builder.append()` で増分適用する。一致しない場合（再送・巻き戻り）は全再構築で適用する（`applyAppend` の重複ガードと同じ性質を行構築でも保つ）
- `RowView` を `React.memo` 化する。ヘッダ・グラフ更新のための親再レンダリングが起きても、参照の変わらない行は再レンダリングされない
- `detail.messages` / `exchanges` / `summary` / `cost` は従来どおり全量差し替え（ヘッダ・やりとり別コストグラフ用）

## 受け入れ基準（Issue #33）

### 行の増分構築（web/src/lib/thread.ts）

- [x] `SPEC-LIVE-060` 追記レコードの増分適用で、変化のない既存行のオブジェクト参照が保たれ、新着行だけが末尾に追加される
- [x] `SPEC-LIVE-061` 追記された tool_result は発行元 assistant 行に取り付けられ、その行だけが新しい参照に置き換わる
- [x] `SPEC-LIVE-062` 追記された sidechain の続きは既存の分岐行に連結され、その行だけが新しい参照に置き換わる
- [x] `SPEC-LIVE-063` 増分適用の結果は同じレコード列を一括で平坦化した結果と一致する

### 差分適用（web/src/lib/bodystore.ts・SessionView）

- [x] `SPEC-LIVE-064` append イベントで表示済み行の DOM 要素は作り直されず、新着行だけが追加される
- [x] `SPEC-LIVE-065` start が既知件数と一致しない append は全再構築で適用され、表示が二重にならない

### E2E（tests/e2e）

- [x] `SPEC-LIVE-066` 追記前に目印を付けた表示済みメッセージの DOM 要素が追記後も同一で、新着分だけが末尾に現れる
- [x] `SPEC-LIVE-067` 追記の前後で、取得済み本文範囲への再リクエストが発生しない
- [x] `SPEC-LIVE-068` 会話の途中へスクロールした状態で追記が来ても、スクロール位置が変わらない
