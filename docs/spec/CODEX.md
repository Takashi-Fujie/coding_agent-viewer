...
担当 Issue: #17（スキーマ調査・フィクスチャ）→ #29（会話正規化）。後続: #31（UI 統合・ライブ更新）→ #30（usage 会計・コスト）。2026-08-09 のオーナー合意で #31 を #30 より先に着手する（#31 は #29 の正規化のみに依存）。#31 の仕様は SPEC-DASH / SPEC-LIVE 側、#28（ソース抽象化）は SPEC-CORE 側。
